...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /usr/local/bin/node "/usr/local/bin/titanium" build --platform iphone --sdk "3.4.0.v20140919162515" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
