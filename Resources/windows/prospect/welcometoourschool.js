exports.open = function(){
	var welcomeToOurSchoolWindow = Ti.UI.createWindow({
		backgroundImage:'images/page-bg.jpg',
		c_winName:'prospect_welcome'
	});
	
	var welcomeToOurSchoolHeaderView = Ti.UI.createView({
	...
	var welcomeToOurSchoolScrollView = Ti.UI.createScrollView({
		layout:'vertical',
		zIndex:1,
		top:(isIOS) ? '67dp':'45dp'
	});
	
	var welcomeToOurSchoolHeaderLabel = Ti.UI.createLabel({
		text:'Welcome',
		font:{
			fontFamily:'Arial',
			fontWeight:'bold',
	...
	welcomeToOurSchoolMenuIcon.addEventListener('click',function(){
		console.log('welcomeToOurSchoolMenuIcon');
		if(menuToggle){
			drawer.showDrawer();
			menuToggle = false;
		}else{
	...
	welcomeToOurSchoolHeaderView.add(welcomeToOurSchoolMenuIcon);
	welcomeToOurSchoolHeaderView.add(welcomeToOurSchoolHeaderLabel);
	
	welcomeToOurSchoolScrollView.add(mainLogoImageView);
	welcomeToOurSchoolScrollView.add(headingLabel);
	welcomeToOurSchoolScrollView.add(welcomeToOurSchoolLabel);
	
	welcomeToOurSchoolWindow.add(welcomeToOurSchoolHeaderView);
